...
curry_career_regular_season_per_game <- curry_web %>% 
  ...
  html_table() %>% 
  clean_names() %>% 
  mutate(season_start_year = as.integer(substr(season, 1, 4))) #Use season_start_year if you want to calculate
...
league_avg_per_game_stats %>% 
  ...
  filter(season_start_year < 2021 & season_start_year >= 1979) %>% 
  ggplot() +
  ...
  annotate("text",
           ...
           label = "Stephen Curry \nDrafted in \n2009",
           fontface = 3) +
  labs(title = "League Average of 3-Point Shot Attempts per Game",
       x = "",
       y = "Attempts per Game") +
