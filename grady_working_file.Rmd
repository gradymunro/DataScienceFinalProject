...
warriors_web <- read_html("https://www.basketball-reference.com/teams/GSW/stats_basic_totals.html")

curry_career_regular_season <- curry_web %>% 
  html_element("#per_game") %>% 
  html_table()

curry_career_playoffs <- curry_web %>% 
  html_element("#div_playoffs_per_game") %>% 
  html_table()

warriors_historical_stats <- warriors_web %>% 
  html_element("#div_stats") %>% 
  html_table()

warriors_historical_stats <- warriors_historical_stats %>% 
  clean_names() %>% 
  discard(~all(is.na(.) | . =="")) %>% 
  filter(season != "Season")
```
